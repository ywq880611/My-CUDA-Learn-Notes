...
// For each block in this kernel, it will assign all elements(traits) for a token, each thread will
// assign one element(trait) for this token.
__global__ void embedding_f32_kernel(const int *idx, float *weight, float *output, int n, int embed_size){
    int bx = blockIdx.x;
    int tx = threadIdx.x;

    int token_idx = idx[bx];
    output[bx * embed_size + tx] = weight[token_idx * embed_size + tx];
}

// For each block in this kernel, it will assign all elements(traits) for a token, each thread will
// assign 4 elements(traits) for this token.
__global__ void embedding_f32x4_kernel(const int *idx, float *weight, float *output, int n, int embed_size){
    int bx = blockIdx.x;
    int tx = threadIdx.x * 4;

    int token_idx = idx[bx];
    output[bx * embed_size + tx] = weight[token_idx * embed_size + tx];
    output[bx * embed_size + tx + 1] = weight[token_idx * embed_size + tx + 1];
    output[bx * embed_size + tx + 2] = weight[token_idx * embed_size + tx + 2];
    output[bx * embed_size + tx + 3] = weight[token_idx * embed_size + tx + 3];
}

// For each block in this kernel, it will assign all elements(traits) for a token, each thread will
// assign 4 elements(traits) for this token.
__global__ void embedding_f32x4_pack_kernel(const int *idx, float *weight, float *output, int n, int embed_size){
    int bx = blockIdx.x;
    int tx = threadIdx.x * 4;

    int token_idx = idx[bx];
    LDST128BITS(output[bx * embed_size + tx]) = LDST128BITS(weight[token_idx * embed_size + tx]);
}
